...
Position Laser::getRelativePosition(int beam) {
    if(beam >= _laser->scan_count)
        std::cout << "[ERROR] Laser::getRelativePosition(" << beam << "), accessing invalid beam!";

    return Position(_laser->point[beam].px, _laser->point[beam].py);
}
